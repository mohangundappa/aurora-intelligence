...
test("presenter path renders events, explanations, identity, and outcome", async ({
  ...
  await expect(page.getByRole("heading", { name: /Stays in Miami/ })).toBeVisible();
  ...
  await page.getByRole("button", { name: /Trigger presenter abandonment/ }).click();

  await page.goto("/console");
  await expect(page.getByRole("heading", { name: /See the why/ })).toBeVisible();
